...
Klog Liveを開いている間は、タイマー割込みがKlogの変更世代を確認し、通常のcompositorを通らない緊急オーバーレイを再描画する。

1. Klogリングを `try_lock` で読む。
2. ロック取得に失敗した場合は待たずにスキップする。
3. 取得できた最新ログを、直接マップ済みのframebufferへ描画する。

通常時は従来通りcompositorがKlog Liveウィンドウを描画する。緊急オーバーレイは最大100列×29行に制限し、ログが更新された時だけ再描画する。
