...
pub fn init_graphics() {
    ...
    if let Some(ref gpu_device) = {
        ...
        petroleum::serial::serial_log(format_args!(
            "[graphics] Dumping BAR registers for device at {}:{}\n",
        ...
        let config_after = nitrogen::pci::PciConfigSpace::read_from_device(
            gpu_device.bus,
            gpu_device.device,
            gpu_device.function,
        )
        ...
        petroleum::serial::serial_log(format_args!(
            "[graphics] Post-enable: Command={:#x}, Status={:#x}\n",
        ...
        petroleum::serial::serial_log(format_args!(
            "[graphics] Dumping first 64 bytes of common_virt+offset ({:#p}):\n",
            common_virt_ptr
        ));
        for i in 0..16 {
        ...
        if let Some(mut gpu) = gpu_result {
            petroleum::serial::serial_log(format_args!(
                "[VirtIO-GPU] Setting up control queue...\n"
            ));
            ...
            if let Some(cfg_cap) = pci_cfg_cap {
                ...
                petroleum::serial::serial_log(format_args!(
                    "[graphics] Found PCI_CFG capability, bar={}, offset={:#x}\n",
                    bar, offset
                ));
                ...
                if let Some(status) =
                    petroleum::virtio::pci::read_virtio_reg_via_pci_cfg(&gpu_device, bar, 0x14, 1)
                {
                    petroleum::serial::serial_log(format_args!(
                        "[graphics] Device status via Type 5: {:#x}\n",
                        status
                    ));
                }
                if let Some(features) =
                    petroleum::virtio::pci::read_virtio_reg_via_pci_cfg(&gpu_device, bar, 0x00, 4)
                {
                    petroleum::serial::serial_log(format_args!(
                        "[graphics] Device features via Type 5: {:#x}\n",
                        features
                    ));
                }
                if let Some(queue_select) =
                    petroleum::virtio::pci::read_virtio_reg_via_pci_cfg(&gpu_device, bar, 0x16, 2)
                {
                    petroleum::serial::serial_log(format_args!(
                        "[graphics] Queue select via Type 5: {:#x}\n",
                        queue_select
                    ));
                }
            }
