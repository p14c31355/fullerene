...
mod tests {
    use super::*;
    use alloc::vec; // vec! マクロを使用するために alloc::vec をインポート
    use alloc::vec::Vec; // Vec 型を明示的にインポート
    ...
    fn test_write_byte_newline_at_end_of_line() {
        ...
        // Writing one more byte should trigger a new line
    ...
    fn test_new_line() {
        let mut mock_buffer = MockVgaBuffer::new(BUFFER_WIDTH, BUFFER_HEIGHT);
        mock_buffer.column_position = BUFFER_WIDTH - 1; // Move to the last column
        mock_buffer.write_byte(b'A');
    ...
    fn test_scrolling() {
        let mut mock_buffer = MockVgaBuffer::new(BUFFER_WIDTH, BUFFER_HEIGHT);
        // Fill the buffer with 'A's to simulate content
        for r in 0..BUFFER_HEIGHT {
            for c in 0..BUFFER_WIDTH {
                mock_buffer.write_byte(b'A');
            }
        }
        // Write a 'B' to trigger scrolling
        mock_buffer.write_byte(b'B');
    ...
    fn test_clear_row() {
        ...
        assert_eq!(char_at_1_0.ascii_character, b'L'); // Line 2 should be untouched
    }
    ...
    fn test_clear_screen() {
        ...
        // Check if the buffer is filled with spaces
        for r in 0..BUFFER_HEIGHT {
            for c in 0..BUFFER_WIDTH {
    ...
    fn test_write_string_non_ascii() {
        let mut mock_buffer = MockVgaBuffer::new(BUFFER_WIDTH, BUFFER_HEIGHT);
        mock_buffer.write_string("Hello€"); // '€' is a non-ASCII character

        let char_h = mock_buffer.get_char(0, 0).unwrap();
        assert_eq!(char_h.ascii_character, b'H');

        let char_euro = mock_buffer.get_char(0, 5).unwrap();
        assert_eq!(char_euro.ascii_character, 0xfe); // Should be replaced by 0xfe
    }
}
