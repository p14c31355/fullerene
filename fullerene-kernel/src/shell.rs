...
pub fn run_wasm_app(path: &str, args: &[&str]) -> i32 {
    ...
    let code = wasi_runtime::runtime::run(
        &binary,
        args,
        wasi_runtime::wasi::WasiHost {
            write_stdout: wasm_write_stdout,
            write_stderr: wasm_write_stderr,
            read_stdin: wasm_read_stdin,
            yield_now: wasm_yield_now,
            wait_for_ns: wasm_wait_for_ns,
            file_size: wasm_file_size,
            read_file_range: wasm_read_file_range,
            read_directory: wasm_read_directory,
            ...
            show_image: wasm_show_image,
            show_text: wasm_show_text,
            show_error: wasm_show_error,
            create_window: wasm_create_window,
...
fn nozzle_services() -> nozzle::ShellServices {
    ...
    let sys = nozzle::sys_hooks::SysHooks {
        info: Some(|ctx, cmd| match cmd {
            ...
            "hbd status" => {
                ctx.terminal
                    .write_str(&crate::drivers::registry::hbd_status());
            }
            "hbd report" => {
                ctx.terminal
