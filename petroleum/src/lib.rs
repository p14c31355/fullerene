    ...
    () => {
        ...
        fn panic(info: &core::panic::PanicInfo) -> ! {
            ...
            #[cfg(feature = "vga_panic")]
            {
                ...
                for i in 0..(80 * 25) { unsafe { vga.add(i).write_volatile(0x4F20u16) }; }
                ...
                if let Some(loc) = info.location() {
                    ...
                    for (i, b) in line_buf.bytes().enumerate() {
                        let off = 80 * 2 + i;
                        if off < 80 * 25 { unsafe { vga.add(off).write_volatile(b as u16 | 0x4F00u16) }; }
                    }
                }
                use core::fmt::Write as _;
...
pub fn clear_line_range<B: TextBufferOperations + ?Sized>(
    buffer: &mut B, start_row: usize, end_row: usize,
    col_start: usize, col_end: usize, blank_char: ScreenChar,
) {
...
pub fn init_uefi_system_table(system_table: *mut EfiSystemTable) {
    let _ = UEFI_SYSTEM_TABLE.lock().insert(UefiSystemTablePtr(system_table));
}
...
#[inline(always)]
pub fn cpu_pause() { core::hint::spin_loop(); }

#[inline(always)]
pub fn cpu_halt() { x86_64::instructions::hlt(); }

pub unsafe fn write_serial_bytes(port: u16, status_port: u16, bytes: &[u8]) {
    #[cfg(not(feature = "std"))]
    unsafe { serial::write_serial_bytes(port, status_port, bytes); }
    #[cfg(feature = "std")]
    {}
}
