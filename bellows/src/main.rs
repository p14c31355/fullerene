...
pub unsafe extern "efiapi" fn efi_main(image_handle: usize, system_table: *mut EfiSystemTable) -> ! {
    if image_handle == 0 { panic!("Invalid image_handle"); }
    ...
    match petroleum::init_graphics_protocols(st) {
        Some(config) => {
            petroleum::bootloader_log!("Graphics framebuffer initialized at {:#x} ({}x{}).", config.address, config.width, config.height);
        }
        None => {
    ...
    if efi_image_size == 0 { panic!("Kernel file is empty."); }
    ...
    let (kernel_phys_start, kernel_entry_phys, entry) = match load_efi_image(
        st, efi_image_file, petroleum::page_table::constants::HIGHER_HALF_OFFSET.as_u64() as usize,
    ) {
        Ok((phys, phys_entry, e)) => {
            petroleum::println!("EFI image loaded successfully. Entry point: {:#p}, Phys entry: {:#x}, Phys base: {:#x}",
                e as *const (), phys_entry, phys.as_u64());
            (phys, phys_entry, e)
        }
        Err(err) => { petroleum::println!("Failed to load EFI image: {:?}", err); panic!("Failed to load EFI image."); }
    };
    ...
    match exit_boot_services_and_jump(image_handle, system_table, kernel_phys_start, kernel_entry_phys, entry) {
        Ok(_) => unreachable!(),
        Err(err) => { petroleum::println!("Failed to exit boot services: {:?}", err); panic!("Failed to exit boot services."); }
    }
}
...
fn install_vga_framebuffer_config(_st: &EfiSystemTable) {
    ...
    let config = FullereneFramebufferConfig {
        address: 0xA0000, width: 800, height: 600,
        pixel_format: EfiGraphicsPixelFormat::PixelFormatMax, bpp: 8, stride: 800,
    };
