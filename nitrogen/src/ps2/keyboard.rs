...
//!
//! Scancode set 1 to ASCII conversion with input buffering and modifier tracking.
//! Provides a `no_std`‑friendly interface usable from interrupt handlers.
//!
//! # Usage
//!
//! 1. Call [`init_keyboard`] once during boot to clear buffers.
//! 2. From the keyboard interrupt handler (IRQ 1), read port `0x60` and pass
//!    the byte to [`handle_keyboard_scancode`].
//! 3. Poll [`read_char`] or [`input_available`] from the scheduler loop or
//!    shell code.
...
/// Keyboard input buffer (byte stream)
static INPUT_BUFFER: Mutex<VecDeque<u8>> = Mutex::new(VecDeque::new());

/// Keyboard input buffer (string / line mode)
...
///
/// Clears any stale input that may have accumulated during boot.
/// Must be called once before enabling keyboard interrupts.
