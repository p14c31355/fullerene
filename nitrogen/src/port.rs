...
//! It depends **only** on the `x86_64` crate — no kernel or boot crate dependencies.
...
macro_rules! port_write {
    ($port_addr:expr, $value:expr) => {{
        let mut writer = $crate::port::PortWriter::new($port_addr);
...
macro_rules! write_port_sequence {
    ($($config:expr, $index_port:expr, $data_port:expr);*$(;)?) => {{
        $(
            let mut vga_ports = $crate::port::VgaPortOps::new($index_port, $data_port);
...
/// VGA register writer for efficient VGA operations (legacy support)
