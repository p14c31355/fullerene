...
impl CodecGraph {
    ...
    pub unsafe fn enumerate(mmio: *mut u8, corb: &CorbEngine, codec: u8) -> Self {
        let vendor_id = unsafe { corb.send_verb(mmio, codec, 0, verbs::GET_PARAM, params::VENDOR_ID) };
        let revision_id = unsafe { corb.send_verb(mmio, codec, 0, verbs::GET_PARAM, params::REVISION_ID) };
        let sub = unsafe { corb.send_verb(mmio, codec, 0, verbs::GET_PARAM, params::SUBORDINATE_COUNT) };
        ...
        if sub != 0xFFFF_FFFF && count_root > 0 {
            ...
            for n in start_root..=end_root {
                let wc = unsafe { corb.send_verb(mmio, codec, n, verbs::GET_PARAM, params::AUDIO_WIDGET_CAP) };
                if wc == 0xFFFF_FFFF {
                    continue;
                }
                let t = (wc >> 20) & 0xF;
        ...
        let sub2 = unsafe { corb.send_verb(
            mmio,
            codec,
            afg_node,
            verbs::GET_PARAM,
            ...
            for n in start_afg..=end_afg {
                ...
                match t {
                    ...
                    | widget_type::AUDIO_SELECTOR => {
                        out_amp_cap = unsafe { corb.send_verb(
                            mmio,
                            codec,
                            n,
                            verbs::GET_PARAM,
                        ...
                        {
                            in_amp_cap = unsafe { corb.send_verb(
                                mmio,
                                codec,
                                n,
                                verbs::GET_PARAM,
                        ...
                        pcm = unsafe { corb.send_verb(mmio, codec, n, verbs::GET_PARAM, params::PCM) };
                        stream = unsafe { corb.send_verb(mmio, codec, n, verbs::GET_PARAM, params::STREAM) };
                    }
                    widget_type::PIN_COMPLEX => {
                        pin_cap = unsafe { corb.send_verb(mmio, codec, n, verbs::GET_PARAM, params::PIN_CAP) };
                        pin_default = unsafe { corb.send_verb(mmio, codec, n, verbs::GET_CONFIG_DEFAULT, 0) };
